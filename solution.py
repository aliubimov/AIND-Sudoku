...
diagonal_units = [[x[0] + x[1] for x in zip(rows, cols)]] + [[x[0] + x[1] for x in zip(rows[::-1], cols)]]
...
def grid_values(grid):
    ...
    for v in grid:
        result.append(v.replace('.', '123456789'))
...
def only_choice(values):
    for unit in unitlist:
        for d in '123456789':
            ...
            if len(dplaces) == 1:
                assign_value(values, dplaces[0], d)
...
def search(values):
    ...
    unsolved_boxes = [box for box in reduced.keys() if len(reduced[box]) > 1]
    ...
    for box in unsolved_boxes:
        chosen_box = box if len(reduced[box]) < len(reduced[chosen_box]) else chosen_box

    for next_try in reduced[chosen_box] :
        new_sudoku = reduced.copy()
        new_sudoku[chosen_box] = next_try
